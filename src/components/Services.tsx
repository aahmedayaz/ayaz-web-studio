...
function VerticalIconTicker({
  ...
  return (
    <div
      aria-hidden
      className={`services-ticker pointer-events-none absolute inset-y-0 z-1 hidden w-22 overflow-hidden lg:block xl:w-25 ${
            ...
            {items.map((item) => {
              ...
              return (
                <div
                  key={`${copy}-${item.id}`}
                  title={item.label}
                  className="services-ticker__icon flex h-14 w-14 items-center justify-center rounded-[1.15rem] bg-white shadow-[0_8px_24px_rgba(0,0,0,0.18)] xl:h-16 xl:w-16 xl:rounded-5"
                >
                  <Logo
    ...
    <section id="services" className="section-pad blueprint-grid">
      <div className="container-page">
        ...
        <div className="relative">
          ...
          <div className="min-w-0 lg:px-22 xl:px-25">
            <div className="grid grid-cols-1 border-t border-(--line) sm:grid-cols-2 lg:grid-cols-3 lg:border-x lg:border-(--line)">
              {SERVICES.map((service, index) => {
                ...
                return (
                  <Reveal
                    key={service.title}
                    as="article"
                    className="group border-b border-(--line) p-6 transition-colors hover:bg-(--surface) sm:border-r sm:nth-[2n]:border-r-0 lg:border-r lg:nth-[2n]:border-r lg:nth-[3n]:border-r-0 md:p-8"
                  >
                    <div className="mb-5 flex items-center justify-between">
                      <span className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-(--line) text-[0.72rem] font-medium text-(--fg-soft)">
                        {num}
                      </span>
                      <Icon
                        size={20}
                    ...
                    <h3 className="mb-2 text-[1.08rem] font-bold tracking-tight text-(--fg)">
                      {service.title}
                    </h3>
